...
contract Loan_Details is Bank,NewSPToken 
{
    ...
    function req(address _token,address bank_address,uint8 year,uint256 tokens)public payable//returns(uint256,uint256)
    {
       ...
       //uint256 amt = (eth * tokens);
        ...
        require(msg.value==tokens&& tokens <=balanceOf[msg.sender]);
        require (bank_d1[bank_address].bal > msg.value );
                                                             
        NewSPToken(_token).transferFrom(msg.sender,bank_address,tokens);
        bank_d1[msg.sender].bal += msg.value;  //   msg.sender.transfer(msg.value);
        bank_d1[bank_address].bal -= msg.value;
        ln_get[msg.sender][ln_get_count[msg.sender]].bank_address = bank_address;
        ln_get[msg.sender][ln_get_count[msg.sender]].amount = msg.value;
        ...
        ln_get[msg.sender][ln_get_count[msg.sender]].installment=(msg.value)/(year*12);
        ln_get[msg.sender][ln_get_count[msg.sender]].bal_ln = msg.value;
        ln_pro[bank_address][ln_pro_count[bank_address]].bank_address = msg.sender;
        ln_pro[bank_address][ln_pro_count[bank_address]].amount = msg.value;
